...
PANDA_TRAIN = r'/home/csgrad/mbhosale/Lidar_Segmentation/pandaset-devkit/data/PandaSet/train/'
PANDA_VALID = r'/home/csgrad/mbhosale/Lidar_Segmentation/pandaset-devkit/data/PandaSet/valid/'
...
def train(args):
    if data_name == 's3dis':
        ...
        model = RandLANet(k=args.neighbors,
            d_in=d_in,
    ...
    with SummaryWriter(logs_dir) as writer:
        for epoch in range(first_epoch, args.epochs+1):
            ...
            for points, labels in tqdm(train_loader, desc='Training', leave=False):
                ...
                losses.append(loss.cpu().item())
                # print(f"Iteration: {i}/{le} Loss: {loss.item()}")
                i +=1
                accuracies.append(accuracy(scores, labels))
                ious.append(intersection_over_union(scores, labels))
...
if __name__ == '__main__':
    ...
    base.add_argument('--dataset', type=Path, help='location of the dataset',
                        default='/home/csgrad/mbhosale/RandLA-Net-pytorch/datasets/s3dis/subsampled')
